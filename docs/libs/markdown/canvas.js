...
export default class Canvas extends React.Component {
  ...
  componentDidMount() {
    this.renderSource(this.source ? this.source[2] : null);
  }
  blockControl() {
    this.setState({
  ...
  render() {
    ...
    return (
      ...
      >
        <div
          className={`${prefixCls}-demo-source`}
          ref="source"
          id={this.playerId}
        />
        <div
          className={`${prefixCls}-demo-meta`}
          style={{
        ...
        >
          {this.description && (
            <div
              ref="description"
              className={`${prefixCls}-demo-des`}
              dangerouslySetInnerHTML={{ __html: this.description }}
            />
          )}
          <div
            ref="highlight"
            className={`${prefixCls}-demo-highlight`}
            dangerouslySetInnerHTML={{ __html: this.highlight }}
          />
        </div>
        {this.state.showBlock ? (
          <div
            className={`${prefixCls}-demo-control`}
